...
router.get('/', auth, async (req, res) => {
  try {
    const { rows } = await db.query(
      'SELECT * FROM routine_backlog WHERE patient_id = $1',
      [req.user.id]
    );
...
router.post('/', auth, async (req, res) => {
  ...
  try {
    const { rows } = await db.query(
      `INSERT INTO routine_backlog (patient_id, items, status, updated_at)
       VALUES ($1, $2, $3, NOW())
...
router.delete('/', auth, async (req, res) => {
  ...
  try {
    await db.query('DELETE FROM routine_backlog WHERE patient_id = $1', [patient_id]);
